...
public class IndustrialGive implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender instanceof Player && command.getName().equalsIgnoreCase("industrialgive")){

            Player p = (Player) sender;
            if (sender.hasPermission("industrial.industrialgive")){
                Player targetPlayer;
                if(args.length == 0){
                    ...
                    ItemStack itemStack = ItemRelated.getItemStack(itemName);
